...
## 摘要
本文以已筛定的109本高质量期刊石油领域文献数据为基础数据源，整合CNKI、WOS与CSCD三库2011-2025年记录，构建覆盖311321篇文献的双主键去重正式语料。研究首先按照“标准DOI键优先、元数据回退键补充”的规则完成跨库去重，并通过机构译名清洗、标准化归并与子单元过滤构建机构分析底表；随后从科研产出、学术影响、合作与国际化三个维度提取7项指标，采用先验权重、熵权法与CRITIC法的组合赋权方案，通过TOPSIS对全球石油领域核心研究机构开展综合评价；最后基于103768篇建模文献，按2011-2015、2016-2020、2021-2025三个阶段构建LDA主题模型，分析研究热点演化路径以及不同机构类型、机构层级的主题偏好差异。结果表明：TOPSIS综合排名前两位分别为中国科学院和中国石油大学（北京），其中中国科学院综合得分为0.793235；机构画像环节共识别7895个有效标准化机构，其中企业研发中心5313家、高校/科研院所2433家、政府机构28家，另有121家归为“其他”；三阶段模型最优主题数分别为12、10和11，共识别14条主题演化路径，研究热点总体呈现出由盆地构造、油田开发与传统催化转化，逐步转向储层预测、二氧化碳利用与储存、催化活性和材料稳定性等方向的趋势。高校/科研院所在知识探索型主题上更具优势，企业研发中心则更集中于工程开发与应用导向主题。研究为石油领域科研机构竞争力诊断、主题布局优化和资源配置决策提供了可复现的数据分析路径。
...
### 2.4 指标体系与组合赋权方法
结合开题报告中“科研产出—学术影响—合作与国际化”三维框架与当前数据可得性，本文最终保留7项正向指标进入综合评价模型，即去重论文总数、近五年发文占比、H指数、高被引论文占比、合作论文占比、国际合作论文占比和合作国家/地区数。由于当前重建过程未恢复出可追溯的专家评分数据，本文不再将德尔菲法作为实际赋权手段，而是采用“维度均衡先验权重+熵权法+CRITIC法”的组合赋权方案。具体而言，先对7项指标进行极差标准化处理，将不同量纲指标压缩到统一量纲区间；若某一指标在全样本中无差异，则该列统一记为1。随后按一级维度等权生成先验权重，即科研产出与学术影响两个维度下各指标先验权重均为1/6，合作与国际化维度下各指标先验权重均为1/9；再基于标准化矩阵分别计算熵权和CRITIC权重，其中熵权反映指标离散度，CRITIC同时考虑标准差与指标间相关冲突。最后将三类权重进行算术平均并再次归一化，形成最终组合权重，以兼顾理论结构约束与样本客观差异。
...
### 2.5 TOPSIS评价与机构分层
在TOPSIS评价阶段，本文并不直接使用极差标准化矩阵排序，而是先对原始正向指标矩阵进行向量归一化处理，再与组合权重相乘构建加权规范化决策矩阵。由于7项指标均为正向指标，正理想解取各指标加权值的最大值，负理想解取最小值；随后计算各机构到正、负理想解的欧氏距离，并据此获得TOPSIS综合贴近度得分。得分越高，表示机构越接近理想最优状态。与此同时，本文还基于极差标准化矩阵和组合权重计算科研产出、学术影响、合作与国际化三个维度的分项得分，用于解释机构“强在何处”。基于综合得分分布，进一步将接入机构画像链条的核心机构划分为头部引领型、中坚创新型和特色细分型三个层级，用于后续机构画像和主题偏好比较。

### 2.6 主题演化分析流程
主题分析以正式语料中的摘要文本为主要对象，按2011-2015、2016-2020、2021-2025三个阶段分别建模。摘要选取遵循“英文摘要优先、中文摘要补充”的原则：若英文摘要存在且长度不少于30个字符，则优先使用英文摘要；否则回退到中文摘要；若中文摘要缺失但英文摘要可用，则仍使用英文摘要。在领域过滤环节，本文并非仅依据摘要判断文献是否属于石油领域，而是将题名、英文题名、中英文关键词与所选摘要合并为综合文本，只要命中石油领域锚词即计入“通过领域过滤文献数”。在建模入样环节，本文继续对所选摘要执行分词、短语合并、术语映射和停用词过滤，并要求清洗后保留的有效词数不少于5；未达到阈值的文献不进入LDA训练。因此，“通过领域过滤文献数”反映主题相关性，“进入建模文献数”反映在相关性基础上同时满足文本有效性的最终样本规模。词袋构建中设置最小文档频次20、最大文档频率比例0.45、最大特征数5000，主题数则根据困惑度与一致性得分联合判定，最终三阶段分别选择12、10和11个主题。随后，本文依据相邻阶段主题词分布余弦相似度构建演化连接，仅保留相似度不低于0.20的路径，以识别主题的延续、分化与迁移关系。
...
由此可见，高校/科研院所更偏向知识探索和机理阐释主题，而企业研发中心则持续围绕储层评价、盆地构造和工程开发等应用导向议题展开布局。政府机构样本经母机构归并和规则修正后已扩展为28家，主要包括部委、地质调查机构和国家技术机构。考虑到其总体规模仍明显小于高校/科研院所与企业研发中心，且少数机构改革前后的历史署名仍保留为相邻标准名，因此其主题分布更适合作为补充观察，而不宜进行强度相当的正式比较。
...
### 4.4 机构层级主题偏好比较
从机构层级看，2011-2015年头部引领型机构的高占比主题为“盆地 / 构造 / 断层 / 地层”（0.1583）、“钻井 / 水力压裂 / 地层 / 水平”（0.1300）和“储层 / 渗透率 / 孔隙 / 孔隙度”（0.1283）；中坚创新型则高度集中于“地层 / 催化剂 / 二氧化碳 / 颗粒”（0.5108），显示出明显的单一优势主题集聚；特色细分型主要分布在“盆地 / 构造 / 断层 / 地层”（0.1680）和“地层 / 催化剂 / 二氧化碳 / 颗粒”（0.1328）等主题。2016-2020年，头部引领型更多覆盖“储层 / 注入 / 钻井 / 渗透率”“裂缝 / 地震 / 应力 / 速度”和“催化剂 / 产品 / 构造 / 催化”三类主题，中坚创新型与特色细分型则对“催化剂 / 产品 / 构造 / 催化”保持更高集中度。到2021-2025年，头部引领型机构更偏向“储层 / 地震 / 模拟 / 裂缝”“孔隙 / 页岩 / 储层 / 渗透率”和“盆地 / 储层 / 断层 / 构造”，而中坚创新型与特色细分型都明显向“构造 / 离子 / 材料 / 稳定性”“催化剂 / 催化 / 活性 / 二氧化碳”聚集。
总体而言，机构层级越高，主题覆盖通常越广，更容易同时连接地质解释、工程预测与低碳转型相关议题；而中坚创新型和特色细分型机构则更依赖少数高占比主题形成差异化优势。这说明影响力层级不仅是结果性排名，也可视为主题布局广度和资源整合能力的一个侧面反映。
...
### 5.5 研究局限
本文仍存在三点局限。第一，机构类型识别虽已显著改善，但仍有121个机构暂列“其他”，说明少量跨国联盟、中心类组织及边界模糊机构的属性辨识仍需进一步细化。第二，由于缺乏可追溯的专家评分数据，本文未将德尔菲法纳入实际赋权流程，而是采用完全可复现的组合客观赋权方案。第三，政府机构识别虽已补入部委、调查机构和国家技术机构，并对可识别子单元完成母机构归并，但少数存在机构改革沿革关系的政府部门仍按历史署名保留为相邻标准名，后续可结合组织连续性进一步统一口径。
