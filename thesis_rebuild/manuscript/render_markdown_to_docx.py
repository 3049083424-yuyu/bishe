...
COVER_UNIVERSITY_TEXT = "中国石油大学（北京）"
COVER_DOC_TYPE_TEXT = "本科毕业论文"
COVER_META_ITEMS = [
    ("学院", "经济管理学院"),
    ("专业", "信息管理与信息系统"),
    ("学生姓名", "余冰雁"),
...
def add_title(doc: Document, text: str) -> None:
    ...
    p_university.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p_university.paragraph_format.space_before = Pt(110)
    p_university.paragraph_format.space_after = Pt(18)
    run_university = p_university.add_run(COVER_UNIVERSITY_TEXT)
    set_run_font(run_university, 22, bold=True, east_asia="黑体")

    p_doc_type = doc.add_paragraph()
    p_doc_type.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p_doc_type.paragraph_format.space_after = Pt(110)
    run_doc_type = p_doc_type.add_run(COVER_DOC_TYPE_TEXT)
    set_run_font(run_doc_type, 18, bold=True, east_asia="黑体")

    p_title = doc.add_paragraph()
    p_title.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p_title.paragraph_format.space_after = Pt(48)
    run_title = p_title.add_run(text)
    set_run_font(run_title, 20, bold=True, east_asia="黑体")

    table = doc.add_table(rows=len(COVER_META_ITEMS), cols=2)
    table.alignment = WD_TABLE_ALIGNMENT.CENTER
    table.autofit = False
    set_table_borders_none(table)

    for idx, (label, value) in enumerate(COVER_META_ITEMS):
        left_cell = table.cell(idx, 0)
        right_cell = table.cell(idx, 1)
        left_cell.width = Mm(35)
        right_cell.width = Mm(85)

        left_para = left_cell.paragraphs[0]
        clear_paragraph(left_para)
        left_para.alignment = WD_ALIGN_PARAGRAPH.RIGHT
        set_paragraph_style(left_para, first_line_indent=False, line_spacing=1.5)
        left_run = left_para.add_run(f"{label}：")
        set_run_font(left_run, 14, bold=True)

        right_para = right_cell.paragraphs[0]
        clear_paragraph(right_para)
        right_para.alignment = WD_ALIGN_PARAGRAPH.LEFT
        set_paragraph_style(right_para, first_line_indent=False, line_spacing=1.5)
        right_run = right_para.add_run(value)
        set_run_font(right_run, 14)

    p_date = doc.add_paragraph()
    p_date.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p_date.paragraph_format.space_before = Pt(60)
    run_date = p_date.add_run(COVER_DATE_TEXT)
    set_run_font(run_date, 14)
...
def add_image(doc: Document, markdown_path: Path, alt_text: str, raw_path: str) -> None:
    ...
    if alt_text.strip():
        add_caption(doc, alt_text.strip())
